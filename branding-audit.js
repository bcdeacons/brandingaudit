 ...
 * Config: window.baConfig = { ajaxUrl: '...', nonce: '...' }
 */
(function (window, document) {
  ...
  var CSS = [
    '#ba-btn{',
      ...
      'width:52px;height:52px;border-radius:50%;background:#7c3aed;',
      'color:#fff;border:none;cursor:pointer;',
      'box-shadow:0 4px 18px rgba(0,0,0,.28);',
      'display:flex;align-items:center;justify-content:center;',
      'transition:transform .2s,background .2s;padding:0;',
    '}',
    '#ba-btn:hover{background:#6d28d9;transform:scale(1.07)}',
    '#ba-panel{',
    ...
    '#ba-header{',
      'padding:14px 18px;background:#7c3aed;color:#fff;',
      'display:flex;align-items:center;justify-content:space-between;flex-shrink:0;',
    '}',
    '#ba-header h2{margin:0;font-size:15px;font-weight:700;letter-spacing:-.2px}',
    '#ba-header span{font-size:12px;opacity:.75;margin-left:8px;font-weight:400}',
    '#ba-close{',
    ...
    '#ba-url-bar{',
      'padding:10px 18px;background:#f5f3ff;border-bottom:1px solid #e2e8f0;flex-shrink:0;',
    '}',
    ...
    '}',
    '#ba-url-input:focus{border-color:#7c3aed;box-shadow:0 0 0 3px rgba(124,58,237,.15)}',
    '#ba-score-bar{',
      'padding:14px 18px;background:#faf8ff;border-bottom:1px solid #e2e8f0;',
    ...
    '#ba-run-btn{',
      'padding:7px 13px;background:#7c3aed;color:#fff;border:none;',
      'border-radius:6px;cursor:pointer;font-size:12px;font-weight:600;',
      'white-space:nowrap;flex-shrink:0;',
    '}',
    '#ba-run-btn:hover{background:#6d28d9}',
    ...
    '}',
    '.ba-section-header:hover{background:#f5f3ff}',
  ...
  function makeChecker(checks, ptsRef) {
    return function check(label, pass, warn, detail, weight) {
      ...
      checks.push({ pass: pass ? 'pass' : warn ? 'warn' : 'fail', label: label, detail: detail });
    };
  }
  ...
  function auditIdentity(doc) {
    ...
    // Apple touch icon (PWA / iOS home screen)
    ...
    // Schema.org Organization or LocalBusiness
  ...
  function auditMessaging(doc) {
    ...
    // Tagline / subheadline (first H2 or large paragraph near page top)
    ...
    check(
      ...
      ctaEls.length === 0
        ...
        : ctaEls.length + ' CTA element(s) found: ' +
            ctaEls.slice(0, 3).map(function (el) { return '\u201c' + textOf(el).slice(0, 25) + '\u201d'; }).join(', '),
      12
    );

    // Contact information (phone or email visible in text)
    ...
    // Value proposition — meaningful paragraph text near the top
  ...
  function auditVisual(doc) {
    ...
    var pts      = { earned: 0, total: 0 };
    var check    = makeChecker(checks, pts);
    var palette  = [];   // for extra rendering
    ...
    // Too many font families (rough check: count distinct font-family rules in style tags)
    ...
    // CSS custom properties (design tokens = brand consistency)
    ...
    // Inline style count (too many = inconsistent styling)
    ...
    // Brand color palette — extract theme-color + og:image dominant check
    var themeColor = doc.querySelector('meta[name="theme-color"]');
    if (themeColor) {
      var tc = attrOf(themeColor, 'content');
      if (tc) palette.push(tc);
    }
    // Extract --color / --primary / --brand variables from :root
    styleEls.forEach(function (s) {
    ...
    // OG image present (visual brand on social)
    ...
    check(
      ...
      ogImage
        ...
        : 'No og:image \u2014 add a branded 1200\xd7630 image for social shares',
      10
    );
  ...
  function auditSocial(doc) {
    ...
    // Schema.org Social Profiles (sameAs array on Organization)
    var schemas    = Array.from(doc.querySelectorAll('script[type="application/ld+json"]'));
  ...
  /* ─── Global open function (for shortcode buttons) ───────────────────── */
  window.baOpenPanel = function () {
    var panel = document.getElementById('ba-panel');
    if (panel) panel.classList.add('ba-open');
  };
  ...
  function buildUI() {
    ...
    btn.addEventListener('click', function () {
      document.getElementById('ba-panel').classList.add('ba-open');
    });
    ...
    panel.innerHTML = [
      ...
      '<div id="ba-url-bar">',
        ...
        '<input type="url" id="ba-url-input" placeholder="https://example.com" autocomplete="off">',
      '</div>',
      '<div id="ba-score-bar">',
      ...
      '</div>',
      '<div id="ba-body">',
        '<div id="ba-placeholder" style="padding:48px 20px;text-align:center;color:#94a3b8">',
  ...
  function onRunAudit() {
    ...
    if (!inputUrl || isSameOrigin(inputUrl)) {
      setTimeout(function () {
        try {
          runAuditOnDoc(document, window.location.href);
        } catch (err) {
  ...
  function fetchAndAudit(url, runBtn) {
    ...
    data.append('action', 'ba_fetch_url');
  ...
  function renderResults(results, auditedUrl) {
    ...
    var totalScore = sections.reduce(function (s, sec) { return s + results[sec.key].score; }, 0);
    ...
    document.getElementById('ba-overall-label').textContent =
      avg >= 80 ? 'Strong brand presence!' : avg >= 50 ? 'Brand needs improvement' : 'Brand consistency issues found';
    ...
    sections.forEach(function (sec) {
      ...
      // Visual section: render color swatches
      ...
      // Social section: render found network tags
