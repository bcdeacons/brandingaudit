...
(function (window, document) {
  ...
  function getCleanAuditDoc() {
    ...
    var nodes = cleanDoc.querySelectorAll(
      '#ba-panel, #ba-widget-style, [data-ba-ignore="true"]'
    );
...
/* 1. Look for :root vars inside <style> tags in the audited doc */
styleEls.forEach(function (s) {
...
/* 2. Fallback for current-page audits:
      read CSS variables from the live documentElement computed styles */
if (palette.length === 0 && window.location.href === (doc.URL || window.location.href)) {
  var liveRootStyles = getComputedStyle(document.documentElement);
  [
    '--color-brand',
    '--brand',
    '--brand-color',
    ...
    addPaletteColor(palette, liveRootStyles.getPropertyValue(varName), 4);
  });
}

/* 3. Final fallback:
      inspect a few likely brand-carrying elements for actual colors */
if (palette.length === 0) {
  var fallbackEls = document.querySelectorAll(
    'a, button, .button, .btn, [class*="btn"], [class*="button"], header, nav'
  );

  for (var i = 0; i < fallbackEls.length && palette.length < 4; i++) {
    var cs = getComputedStyle(fallbackEls[i]);
    addPaletteColor(palette, cs.backgroundColor, 4);
    addPaletteColor(palette, cs.color, 4);
    addPaletteColor(palette, cs.borderColor, 4);
  }
}
...
check(
  ...
  false,
  palette.length > 0
    ? palette.length + ' brand color(s) detected via CSS variables, theme-color, or rendered styles'
    : 'No core brand colors found — define variables like --color-brand, --color-primary, or --color-accent in :root',
  8
);

return {
  score: Math.round((pts.earned / pts.total) * 100),
  checks: checks,
  palette: palette
};
}
  ...
  function buildUI() {
    ...
    panel.innerHTML = [
      ...
      '<div id="ba-score-bar">',
        ...
        '</div>',
        '<button id="ba-run-btn" type="button">Run Audit</button>',
      '</div>',
      '<div id="ba-body">',
...
if (runBtn) {
  ...
  var inputUrl = normalizeUrl(rawUrl);
  ...
  function finishRun() {
    baIsRunning = false;
    var btn = document.getElementById('ba-run-btn');
    if (btn) {
      btn.disabled = false;
  ...
  // If no URL is entered, audit the current loaded page DOM
  if (!inputUrl) {
    setTimeout(function () {
      try {
        var cleanDoc = getCleanAuditDoc();
        runAuditOnDoc(cleanDoc, window.location.href);
      } catch (err) {
  ...
  // If a URL is entered, always fetch fresh HTML
  fetchAndAudit(inputUrl, finishRun);
}

  function fetchAndAudit(url, done) {
  if (!cfg.ajaxUrl) {
  ...
  fetch(cfg.ajaxUrl, {
    ...
    .then(function (json) {
      if (!json.success) {
        throw new Error(json.data || 'Failed to fetch URL');
      }
